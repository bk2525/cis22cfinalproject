...
public class FileHandler implements AutoCloseable {
    ...
    /**
     * Loads all matching data files into memory from a directory;
     * files are matched by prefix and suffix, where any text in
     * between the two is treated like a wildcard:
     * E.g., if prefix = "song" and suffix = ".txt", then
     *       song*.txt would match, where * is any character sequence
     *
    ...
    public String readDir(
        ...
        if (files == null || files.length == 0) {
            throw new FileNotFoundException(String.format(
                ...
                + "the files are inaccessible for reading."
    ...
    public String readFile(File file)
        ...
        try (Scanner fileScan = new Scanner(file)) {
            ...
            throw new FileNotFoundException(String.format(
                "readFile(): " + file + " cannot be found or is "
                + "inaccessible for reading.%n"
                + "  Exception: %s%n",
                fnfe.getMessage()));
        }
    ...
    /**
     * Insertion sorts a list of files by name;
     * Sorts in natural ascending order by file name
     *
     * @throws NullPointerException if file list is null or empty
     */
    public void sortFilesByName(File[] files)
        throws NullPointerException {

        if (files == null || files.length == 0) {
            throw new NullPointerException(
                "sortFilesByName(): The files list is null or empty.");
        }
    ...
    /**
     * Compares two files by name length, and then lexicographically
     * by file name if needed (e.g., song2.txt should come
     * before song10.txt); Note that nulls are moved to the end of a
     * list if calling this to perform a sort, additionally this
     * method also assumes that the files will only differ at the
     * numeric identifier between the file prefix and suffix.
     *
